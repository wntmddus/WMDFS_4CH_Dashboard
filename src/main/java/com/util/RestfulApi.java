...
public class RestfulApi {
    ...
    public static String postBatch(String path, List<JSONObject> requestBodyList) throws IOException {
//        System.out.println(requestBodyListy.toString());
        PowerShellResponse response = null;
        try (PowerShell powerShell = PowerShell.openSession()) {
            //Increase timeout to give enough time to the script to finish
            Map<String, String> config = new HashMap<String, String>();
            ...
            for (int j = 0; j < requestBodyList.size(); j++) {
                String s = requestBodyList.get(j).toString();
                StringBuilder str = new StringBuilder();
                for (int i = 0; i < s.length(); i++) {
                    if (s.charAt(i) == '"') {
                        str.append('`').append(s.charAt(i));
...
//            powerShell.close();
        ...
        } catch(PowerShellNotAvailableException | InterruptedException ex) {
            //Handle error when PowerShell is not available in the system
            //Maybe try in another way?
            log.info(ex);
            System.err.println(ex);
